<x-layouts::app :title="__('Neuen Quellenbeleg erstellen')">
    <div class="flex h-full w-full flex-1 flex-col gap-4">
        <!-- Header -->
        <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-900">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold mb-2">Neuen Quellenbeleg erstellen</h1>
                    <p class="text-neutral-600 dark:text-neutral-400">Verknüpfen Sie eine neue Quelle mit einem Fall</p>
                </div>
        ...
        <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-900 max-w-2xl">
            <form method="POST" action="{{ route('source-evidence.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="case_id" name="case_id" value="{{ old('case_id') }}" required
                ...
                <div>
                    ...
                    <input type="text" id="source_type" name="source_type" value="{{ old('source_type') }}"
                        class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <!-- Jurisdiction Scope -->
                <div>
                    <label for="jurisdiction_scope" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                        Jurisdiktionsbereich
                    </label>
                    <input type="text" id="jurisdiction_scope" name="jurisdiction_scope" value="{{ old('jurisdiction_scope') }}"
                        class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <!-- Excerpt -->
                <div>
                    <label for="evidence_excerpt" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                        Auszug aus der Quelle
                    </label>
                    <textarea id="evidence_excerpt" name="evidence_excerpt" rows="4"
                        class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Relevanter Text aus der Quelle...">{{ old('evidence_excerpt') }}</textarea>
                </div>

                <!-- Claim Supported -->
                <div>
                    <label for="claim_supported" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                        Behauptung unterstützt
                    </label>
                    <textarea id="claim_supported" name="claim_supported" rows="2"
                        class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Was wird durch diese Quelle belegt?">{{ old('claim_supported') }}</textarea>
                </div>

                <!-- Scores -->
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <label for="authority_score" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                            Autorität (0-100)
                        </label>
                        <input type="number" id="authority_score" name="authority_score" value="{{ old('authority_score') }}" min="0" max="100"
                            class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label for="relevance_score" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                            Relevanz (0-100)
                        </label>
                        <input type="number" id="relevance_score" name="relevance_score" value="{{ old('relevance_score') }}" min="0" max="100"
                            class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                    <div>
                        <label for="jurisdiction_score" class="block text-sm font-medium text-neutral-900 dark:text-white mb-2">
                            Jurisdiktion (0-100)
                        </label>
                        <input type="number" id="jurisdiction_score" name="jurisdiction_score" value="{{ old('jurisdiction_score') }}" min="0" max="100"
                            class="w-full px-4 py-2 border border-neutral-300 rounded-lg dark:border-neutral-600 dark:bg-neutral-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" id="selected" name="selected" value="1" {{ old('selected') ? 'checked' : '' }}
