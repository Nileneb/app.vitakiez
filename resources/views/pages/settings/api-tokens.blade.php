<section class="w-full">
    ...
    <x-pages::settings.layout :heading="$heading ?? __('API Tokens')" :subheading="$subheading ?? ''">
        <div class="flex h-full w-full flex-1 flex-col gap-4">
            ...
            @if (session('new_token'))
                <div class="rounded-xl border border-green-200 bg-green-50 p-6 dark:border-green-900 dark:bg-green-900/20">
                    ...
                    <div
                        class="bg-white dark:bg-neutral-800 p-4 rounded border border-green-300 dark:border-green-700 font-mono text-sm break-all">
                        {{ session('new_token') }}
                    </div>
                    <p class="text-xs text-green-700 dark:text-green-300 mt-3">
                        💡 Use this token in n8n as: <code
                            class="bg-green-100 dark:bg-green-900 px-2 py-1 rounded">Authorization: Bearer {{ session('new_token') }}</code>
                    </p>
                </div>
            @endif
            ...
            <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <form method="POST" action="{{ route('settings.api-tokens.create') }}"
                    ...
                    <input type="text" name="name" placeholder="e.g., n8n, Mobile App, etc." required
            ...
            <div class="rounded-xl border border-neutral-200 bg-white p-6 dark:border-neutral-700 dark:bg-neutral-900">
                ...
                @forelse (auth()->user()->tokens()->latest()->get() as $token)
                    <div
                        class="flex items-center justify-between p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg mb-3">
                        <div>
                        ...
                        <form method="POST" action="{{ route('settings.api-tokens.revoke', $token->id) }}"
            ...
            <div class="rounded-xl border border-blue-200 bg-blue-50 p-6 dark:border-blue-900 dark:bg-blue-900/20">
                ...
                <ol class="list-decimal list-inside space-y-2 text-sm text-blue-800 dark:text-blue-200">
                    ...
                    <li>Add header: <code
                            class="bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded">Authorization: Bearer YOUR_TOKEN</code>
                    </li>
